...
// dbauth.Provider; provider-wide defaults (region, role, ...) live on the
// extension's own config, and the inline value overrides those defaults for this
// consumer only. At Start the component calls Config.GetProvider with the host
// extension map to resolve the dbauth.Provider and the inline override, mirroring
// how config/configauth resolves an authenticator extension via
// Config.GetServerAuthenticator.
...
//
// The named extension must be declared in the extensions block and implement
// dbauth.Provider. GetProvider resolves it from the host extension map and
// returns the inline value as extensionArgs, which the provider merges over its
// configured defaults for this consumer only.
type Config struct {
	// ProviderConfigs holds the inline provider override, keyed by the provider
	// extension's component ID. The ",remain" tag captures every key in the db_auth
	// block; at most one is expected (enforced by Validate). It holds raw config,
	// not Provider instances — it is exported only because mapstructure cannot
	// populate unexported fields. Treat it as read-only and prefer the
	// IsEmpty/Validate/GetProvider methods.
...
// override (extensionArgs) the consumer supplied under the provider's ID. It is
// called from a consuming component's Start, mirroring
// configauth.Config.GetServerAuthenticator.
//
// The returned extensionArgs is the raw value nested under the provider ID key
// (nil when the key has no body); the consumer threads it verbatim into
// dbauth.Provider.GetCredential, where the provider merges it over its own
// defaults. It returns an error when no provider is configured, when more than
// one is configured, when the named extension is not present, or when the named
// extension does not implement dbauth.Provider.
